...
public class Algorithms {
	...
	public static int Split(int[] A, int first, int last)
	{
		...
		int mid = (leftSrch+rightSrch)/2; //int of middle value between left
											//and right.
  
        temp = A[first];      // Swap the middle value
        A[first] = A[mid];    // with the first value
        A[mid] = temp;
        
		Pivot = A[first]; // The Pivot is the middle element of the region.
