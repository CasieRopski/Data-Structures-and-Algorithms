 ...
 * @author (displayBkwrd) Casie Ropski
 * @Description:
...
/*		strList.addToFront("D"); //last one
		strList.addToFront("C"); 
		strList.addToFront("B");
		strList.addToFront("A"); //first one
*/
...

	...
	public int lengthRecursive(Node ptr)
	{
		...
		if (ptr != null) {     // INDUCTIVE STEP, while not at the end 
				count++;	   // at the end of the list. 
				ptr = ptr.next;              // Move to the next node.
				count+= lengthRecursive(ptr); // Add this node to the count.
		} 		
			return count; // BASE STEP is when pointer == null, 
	...
	/* 	displayBkwd() - Display the contents of the list that starts
	 * 					at the node pointed to bt ptr. The list items
	 * 					are displayed in reverse order. A recursive
	 * 					implementations is used.
	 */
	
	private void displayBkwd(Node ptr)
	{
		if (ptr != null) {  				//INDUCTIVE STEP
			displayBkwd(ptr.next);			// Call displayBkwd until last node.
			System.out.println(ptr.data); // Print the data of the ptr.
		} 
		
		// Implied BASE STEP, when ptr equals null, code does nothing.
