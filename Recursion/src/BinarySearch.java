...
public class BinarySearch {
	// Added values from lab paper to array.
	...
	public static int SearchBinRecursive(int[] A, int key, int left, int right)
	{
	    ...
	    mid = (left + right)/2;		//Equation for mid
	    
	    if (left <= right && !found){			// Substitute for while statement in iterative.
	    	if (A[mid] == key) {	//BASE STEP, where the key is identified as mid.
	    		found = true;
	    		return mid;
	    	} else if (key < A[mid]){	//INDUCTIVE STEP, check if key is < or > mid and 
	    							    //alters Right and Left recursively depending on
	    							    //the result. 
